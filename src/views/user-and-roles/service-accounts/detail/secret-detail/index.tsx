...
const SecretDetailMain = () => {
  ...
  const editservicesAccountsSecretDetail = (index: number) => {
    navigate(`./edit/${index}`);
  };
  ...
  return (
    <div className="element-wrapper">
      <div className="shadow-sm p-2 d-flex justify-content-between">
        <h5 className="fw-bold text-black m-0">
          {store?.detail.secrets[0].name}
        </h5>
        <div>
          <div className="d-flex justify-content-end">
            <div className="d-flex me-1">
              ...
              <Button
                className="d-flex align-items-center justify-content-center gap-50"
                onClick={() => editservicesAccountsSecretDetail(store?.id as number)}
              >
      ...
      <div className="p-2 m-2 border rounded">
        <MetadataCard
          age={store?.detail.secrets[0].detail.metadataSecret.age || ''}
          annotation={
            store?.detail.secrets[0].detail.metadataSecret.annotation || []
          }
          created={store?.detail.secrets[0].detail.metadataSecret.created || ''}
          name={store?.detail.secrets[0].detail.metadataSecret.name || ''}
          namespace={
            store?.detail.secrets[0].detail.metadataSecret.namespace || ''
          }
          uid={store?.detail.secrets[0].detail.metadataSecret.uid || ''} />
        <DataCard />
      </div>
    </div>
  );
...
SecretDetailMain.displayName = 'SecretDetailMain';
export { SecretDetailMain };
