...
const NextAPIBranding = () => {
  const [store, dispatch] = useStore(
    (state) => state?.app?.settings?.nextAPIBranding?.main
  );

  const [activeSwitchIndex, setActiveSwitchIndex] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    dispatch(StoreCommand.app.settings.nextAPIBranding.GetMainData());
  }, [dispatch]);
    ...
    <div className="element-wrapper">
      <div className="d-flex justify-content-between align-items-center mb-2 shadow-sm p-2">
        ...
        <Button
          ...
          href={allRoutes.settings.addNextAPIBranding}
        >
          <Plus size={16} />
          Branding
        </Button>
...
NextAPIBranding.displayName = 'NextAPIBranding';
export { NextAPIBranding };
