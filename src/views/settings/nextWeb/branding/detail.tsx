...
import type { NextWebBrandingDataModel } from '@models/app/settings/nextWeb/branding/types';
import { StoreCommand, useStore } from '@models/store';

import { classNameCheck, classNameControl } from '../../constants';

const NextWebDetailBranding = () => {
  const { id } = useParams<{ id?: string }>();

  const [store, dispatch] = useStore(
    (state) => state?.app?.settings?.nextWebBranding?.detail
  );
  ...
  useEffect(() => {
    if (
      ...
      dispatch(StoreCommand.app.settings.nextWebBranding.GetDetailData());
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const [stateData, setStateData] = useState<
  Partial<NextWebBrandingDataModel> | undefined
  >(store);
        ...
        {type === 'Add' || type === 'Edit'
          ...
          <div className="d-flex gap-2">
            <Button
              ...
              href={allRoutes.settings.nextWebBranding}
            >
              <X size={20} /> Cancel
            </Button>
            <Button
...
NextWebDetailBranding.displayName = 'NextWebDetailBranding';
export { NextWebDetailBranding };
