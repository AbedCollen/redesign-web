...
import type { NextIDBrandingDataModel } from '@models/app/settings/nextID/branding/types';
import { StoreCommand, useStore } from '@models/store';

import { classNameCheck, classNameControl } from '../../constants';

const NextIDDetailBranding = () => {
  const { id } = useParams<{ id?: string }>();

  const [store, dispatch] = useStore(
    (state) => state?.app?.settings?.nextIDBranding?.detail
  );
  ...
  useEffect(() => {
    if (
      ...
      dispatch(StoreCommand.app.settings.nextIDBranding.GetDetailData());
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const [stateData, setStateData] = useState<
  Partial<NextIDBrandingDataModel> | undefined
  >(store);
        ...
        {type === 'Add' || type === 'Edit'
          ...
          <div className="d-flex gap-2">
            <Button
              ...
              href={allRoutes.settings.nextIDBranding}
            >
              <X size={20} /> Cancel
            </Button>
            <Button
...
NextIDDetailBranding.displayName = 'NextIDDetailBranding';
export { NextIDDetailBranding };
