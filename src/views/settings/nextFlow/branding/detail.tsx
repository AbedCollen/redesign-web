...
import type { NextFlowBrandingDataModel } from '@models/app/settings/nextFlow/branding/types';
import { StoreCommand, useStore } from '@models/store';

import { classNameCheck, classNameControl } from '../../constants';

const NextFlowDetailBranding = () => {
  const { id } = useParams<{ id?: string }>();

  const [store, dispatch] = useStore(
    (state) => state?.app?.settings?.nextFlowBranding?.detail
  );
  ...
  useEffect(() => {
    if (
      ...
      dispatch(StoreCommand.app.settings.nextFlowBranding.GetDetailData());
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const [stateData, setStateData] = useState<
  Partial<NextFlowBrandingDataModel> | undefined
  >(store);
        ...
        {type === 'Add' || type === 'Edit'
          ...
          <div className="d-flex gap-2">
            <Button
              ...
              href={allRoutes.settings.nextFlowBranding}
            >
              <X size={20} /> Cancel
            </Button>
            <Button
...
NextFlowDetailBranding.displayName = 'NextFlowDetailBranding';
export { NextFlowDetailBranding };
