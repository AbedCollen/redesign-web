...
const ConfigmapMain = () => {
  const [store, dispatch] = useStore(
    (state) => state?.app?.projectManagement?.configmap?.main
  );

  useEffect(() => {
    dispatch(StoreCommand.app.projectManagement.configmap.GetMainData());
  }, [dispatch]);
  ...
  const editConfigMap = (id: number) => {
    navigate(`./editConfigMap/${id}`);
  };
  ...
  return (
    <div className="p-2 element-wrapper">
      <h4 className="fw-bold mb-2 text-black">Config Map</h4>
        ...
        <Row className="mb-1">
          ...
          <Col className="d-flex justify-content-end">
            <Button
              ...
              href={allRoutes.projectManagement.addConfigMap}
            >
              <Plus size={16} />
              Config Map
            </Button>
...
ConfigmapMain.displayName = 'DeploymentMain';
export { ConfigmapMain };
