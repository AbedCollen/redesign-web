...
const SecretMain = () => {
  const [store, dispatch] = useStore(
    (state) => state?.app?.projectManagement?.secret?.main
  );

  useEffect(() => {
    dispatch(StoreCommand.app.projectManagement.secret.GetMainData());
  }, [dispatch]);
  ...
  const editSecretForm = (id: number) => {
    navigate(`./edit-secret-form/${id}`);
  };
  ...
  return (
    <div className="p-2 element-wrapper">
      <h4 className="fw-bold mb-2 text-black">Secret</h4>
        ...
        <Row className="mb-1">
          ...
          <Col className="d-flex justify-content-end">
            <Button
              ...
              href={allRoutes.projectManagement.addSecretForm}
            >
              <Plus size={16} />
              Secret
            </Button>
...
SecretMain.displayName = 'SecretMain';
export { SecretMain };
